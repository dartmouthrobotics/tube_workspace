...
int main(int argc, char **argv) {
  ...
  while (true) {
    ROS_INFO("Waiting for UDP broadcast...");
    ...
    tv.tv_sec = 1;
    tv.tv_usec = 0;
    setsockopt(sockUDP, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
  ...
  // Initialize to ensure we publish first ping
  unsigned int latest_id = 0;

  // Send Ping and initiate data collection
  ROS_INFO("Sending initial Fire command: mode=%d, range=%.1f, gain=%.1f", mode, range, gain);
  sonar.Fire(mode, ping_rate, range, gain, soundspeed, salinity);
  ROS_INFO("Fire command sent");
  ...
  while (ros::ok()) {
    ...
    unsigned int rawSize = sonar.m_readData.m_osBuffer[0].m_rawSize;
