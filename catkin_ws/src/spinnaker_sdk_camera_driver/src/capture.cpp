    ...
    for (int j=0; j<cam_ids_.size(); j++) {
        ...
        for (int i=0; i<numCameras_; i++) {
            ...
            if (cam.get_id().compare(cam_ids_[j]) == 0) {
                ...
                compressed_image_pubs.push_back(nh_.advertise<sensor_msgs::CompressedImage>("camera_array/"+cam_names_[j]+"/image_raw/compressed", 1));
...
void acquisition::Capture::export_to_ROS() {
    ...
    for (unsigned int i = 0; i < numCameras_; i++) {
        ...
        comp_msg.header = img_msg_header;
        if(color_) {
            comp_msg.format = "jpeg";
            std::vector<int> params = {cv::IMWRITE_JPEG_QUALITY, 80};
            cv::imencode(".jpg", frames_[i], comp_msg.data, params);
        } else {
            comp_msg.format = "png";
            cv::imencode(".png", frames_[i], comp_msg.data);
        }
        compressed_image_pubs[i].publish(comp_msg);
